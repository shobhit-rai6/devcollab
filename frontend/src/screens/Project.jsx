...
// ─── Normalise a fileTree value to a plain string ────────────────────────────
// FIX 1: AI returns { file: { contents: "..." } }, DB returns the same after
// JSON round-trip, but sometimes a plain string slips through.
// This helper always returns a string so the editor never crashes.
...
// ─── Normalise an entire fileTree from any source ────────────────────────────
// Ensures every entry is { file: { contents: string } }
...
const Project = () => {
    ...
    // FIX 2: Track which message IDs we've already persisted to DB so we
    // never double-save the same message (the root cause of the x2 on refresh)
    const persistedMsgKeys  = useRef(new Set())
    const isSavingRef       = useRef(false)
    const fileTreeRef       = useRef(fileTree)
    const messagesEndRef    = useRef(null)
    const typingTimerRef    = useRef(null)
    // FIX 3: single source-of-truth flag for history load — prevents the
    // duplicate loadHistory useEffect from running twice
    ...
    // ── Mount files from AI response ─────────────────────────────────────────
    const retryMountFiles = useCallback(async () => {
        if (!webContainer || !aiResponse?.fileTree) return
        try {
            const mountable = normaliseFileTree(aiResponse.fileTree)
            await webContainer.mount(mountable)
            setFileTree(mountable)
            await saveFileTree(mountable)
            const firstFile = Object.keys(mountable)[0]
            if (firstFile) {
    ...
    const WriteAiMessage = useCallback((msg) => {
        ...
        try {
            ...
            return (
                <div className="ai-message ai-text">
                    <Markdown children={String(msg)} options={{ overrides: { code: { component: SyntaxHighlightedCode } } }} />
                </div>
            )
        }
        ...
        if (parsed.type === 'code' || parsed.type === 'mixed') {
            return (
                <div className="ai-message">
                        ...
                        <div style={{ marginTop: '1rem', padding: '0.75rem', background: 'rgba(99,102,241,0.08)', borderRadius: '8px', border: '1px solid rgba(99,102,241,0.15)' }}>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                                {Object.keys(parsed.fileTree).map((file, i) => (
                                    <span key={i} onClick={() => {
                                        // FIX 1: When clicking a file from AI message, normalise the
                                        // entire fileTree first so getFileContents always finds content
                                        const normalised = normaliseFileTree(parsed.fileTree)
                                        setFileTree(normalised)
                                        saveFileTree(normalised)
                                        setAiResponse({ fileTree: parsed.fileTree })
                            ...
                            <button onClick={retryMountFiles}
                    ...
                    {parsed.startCommand && <div style={{ marginTop: '0.5rem', padding: '0.3rem 0.75rem', background: 'rgba(59,130,246,0.1)', borderRadius: '4px', fontSize: '0.7rem', color: '#60a5fa', display: 'inline-block' }}>{parsed.startCommand}</div>}
                </div>
            )
        }
        ...
        return (
            <div className="ai-message ai-text">
                <Markdown children={parsed.content || String(msg)} options={{ overrides: { code: { component: SyntaxHighlightedCode, props: { className: 'hljs' } }, pre: { props: { className: 'ai-pre' } } } }} />
            </div>
        )
    }, [handleFileClick, getFileIcon, retryMountFiles, project?.name, saveFileTree])
    ...
    const send = async () => {
        ...
        if (isAICall && aiAssistant) {
            const clean = userMessage.replace(/^@ai\s*/i, '').replace(/^\/ai\s*/i, '').replace(/^ai:\s*/i, '').trim()
            setIsAiTyping(true)
    ...
    const handleMessageChange = (e) => {
        ...
        setSuggestedCommands(e.target.value.toLowerCase().includes('@ai') ? [
    ...
    const handleAIAction = useCallback(async (action, selectedText) => {
        setAiContextMenu(c => ({ ...c, visible: false }))
        const commands = {
    ...
    // ── Persist a single message (deduped) ──────────────────────────────────
    // FIX 2: key on sender+message+approx-second so identical messages from
    // the same sender within 1 second are never double-written to DB.
    const persistMessage = useCallback(async (data) => {
        const ts = Math.floor(Date.now() / 1000)
        const key = `${data.sender?._id}::${data.message}::${ts}`
        if (persistedMsgKeys.current.has(key)) return
        persistedMsgKeys.current.add(key)
        // Evict keys older than 5 seconds to keep the Set small
        setTimeout(() => persistedMsgKeys.current.delete(key), 5000)
        try {
            await axios.post('/projects/messages', {
                projectId: project._id,
                messages: [{ sender: data.sender, message: data.message, type: data.sender?._id === 'ai' ? 'ai' : 'user' }]
            })
        } catch (err) {
    ...
    useEffect(() => {
        ...
        const fetchProject = async () => {
            try {
                ...
                // FIX 1: normalise file tree on load
    ...
    // Load chat history — FIX 2: SINGLE effect using a ref flag so it fires
    // exactly once. The original code had TWO separate loadHistory useEffects
    // which both ran and loaded the same messages, causing the x2 on refresh.
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        receiveMessage('project-message', async (data) => {
            setMessages(prev => {
                // FIX 2: dedup in state — skip if we already have this exact
                // message within a 2-second window (covers re-renders & strict mode)
            ...
            // Only the RECIPIENT (not the sender) persists to DB.
            // The sender's server.js already persisted it on emit.
            // FIX 2: This is the main fix — previously EVERY socket message
            // was persisted by every connected client, so your own message
            // was written twice (once by server, once by your browser).
            // Now we only persist messages from OTHER senders.
        ...
        // Typing indicators
        receiveMessage('user-typing',      ({ email }) => setTypingUsers(prev => new Set([...prev, email])))
        receiveMessage('user-stop-typing', ({ email }) => setTypingUsers(prev => { const s = new Set(prev); s.delete(email); return s }))

        // File tree sync from other collaborators
    ...
    // ── Derived values ───────────────────────────────────────────────────────
    // FIX 1: always get file contents through the helper, never raw
                ...
                <section className="proj-left">
                    ...
                    {isSearchOpen && (
                        <div style={{ padding: '0.5rem 0.875rem', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            ...
                            <button onClick={() => { setSearchQuery(''); setIsSearchOpen(false) }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#64748b', display: 'flex' }}>
                    ...
                    <div className="messages-area" ref={messageBox}>
                        ...
                        {filteredMessages.map((msg, index) => {
                            ...
                            const isCurrentUser = msg.sender?._id === user?._id?.toString() || msg.sender?.email === user?.email
                            return (
                            ...
                            <div className="msg-wrapper other" style={{ opacity: 0.7 }}>
                                ...
                                <div className="msg-bubble other" style={{ padding: '0.5rem 0.9rem' }}>
                                    <span style={{ display: 'flex', gap: '3px', alignItems: 'center' }}>
                                        {[0,1,2].map(i => <span key={i} style={{ width:6, height:6, borderRadius:'50%', background:'#64748b', animation:`typingDot 1.2s ${i*0.2}s ease-in-out infinite` }}></span>)}
                                    </span>
                                </div>
                            </div>
                        )}
                ...
                <section className="proj-right">
                    ...
                    <div className="code-editor-pane">
                        ...
                        <div className="code-content">
                            {/* FIX 1: Use pre-computed currentFileContents + highlightedCode
                                so we never call hljs.highlight on undefined/object values */}
                        ...
                        <div className="preview-panel">
                            <div className="preview-bar">
                                <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '0.4rem', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '7px', padding: '0.35rem 0.7rem' }}>
                                    ...
                                    <input style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', fontFamily: "'JetBrains Mono',monospace", fontSize: '0.72rem', color: '#94a3b8' }} type="text" value={iframeUrl} onChange={e => setIframeUrl(e.target.value)} />
                                </div>
                            </div>
                ...
                <div className="modal-overlay" onClick={e => e.target === e.currentTarget && (setIsModalOpen(false), setSelectedUserId(new Set()))}>
                    <div className="modal-box">
                        <div className="modal-head">
                            ...
                            <button onClick={() => { setIsModalOpen(false); setSelectedUserId(new Set()) }}
                                ...
                                return (
                                    <div key={u._id}
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            ...
                                            <div style={{ fontSize: '0.68rem', color: '#94a3b8', marginTop: '0.15rem' }}>{isOwner ? 'Project owner' : isAlreadyCollab ? 'Already a collaborator' : 'Click to select'}</div>
                                        </div>
